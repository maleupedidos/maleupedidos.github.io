...
| `probar_deuda_pie.js` | **Los cuadritos de Inicio y la deuda con proveedores** (15/9/2026). `node probar_deuda_pie.js [390|1440]`, sin token: backend stubbeado y el reloj en el lunes 14/9 (semana 38). Sostiene los 7 cuadritos (sin "Cobrado mes", con "Margen mes" = facturado − costo de los entregados del mes), que el pie de la deuda diga **la semana más vieja con algo pendiente** (una ya pagada no cuenta) igual en Inicio y en Cierre > Estado patrimonial, y las dos fases de **`busqueda` con error**: sin copia y con una copia mala ya guardada, ninguna pantalla dice "$0 · al día", las dos dicen "no se pudo calcular", el `{ok:false}` no se guarda y no se reintenta en loop. Contra v342: 22 ok / 8 mal. **Ojo**: el Cierre no dibuja nada sin `action=ventas` (necesita `VD` y un facturado del mes) — sin ese stub el chequeo de Cierre daba `null` y no era del ERP |
| `probar_pago_no_traba.js` | **Un pago a proveedor no deja el ERP trabado** (15/9/2026). `node probar_pago_no_traba.js [390|1440]`, sin token: backend stubbeado, `action=busqueda` demorado 15 s despues del pago. El caso real: el pago a Sevuchitas se registro, la lista tardo mas de 12 s, el ERP la descarto y dejo el formulario ABIERTO con el monto; `_hayEditorAbierto()` lo leia como "editando" y la Caja traia la caja nueva y dibujaba la vieja. Sostiene que el formulario se cierra apenas el servidor confirma, que la lista que llega tarde se aplica, que Actualizar en Caja dibuja la caja nueva ($500.000 -> $400.000), que un pago a medio tipear en OTRA tab no congela la Caja (y lo tipeado sigue ahi), y que un pago rechazado deja el formulario con el monto. Contra v343: 8 ok / 5 mal. **Ojo**: Abastecimiento no dibuja nada sin `ts` en la respuesta de `busqueda` — sin eso la vista de pagos sale vacia y el test no mide nada |
